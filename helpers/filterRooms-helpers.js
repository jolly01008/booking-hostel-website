...
// 關鍵字搜尋，所有房間==========================
const keywordFilter = async keyword => {
...
// 日期(判斷是否已被預約過)、人數篩選搜尋=========================
const dateFilter = async (checkin, checkout, adults, kids) => {
  ...
  // 獨立套房
    if (
      ...
      dayjs(checkin).isSame(bookingData.checkoutDate) ||
      dayjs(checkout).isSame(bookingData.bookingDate) ||
    ...
    dayjs(checkin).isSame(bookingData.checkoutDate) === false &&
    dayjs(checkout).isSame(bookingData.bookingDate) === false &&
