...
const bedsAmountFilter = async (checkin, checkout, adults, kids, roomId) => {
  // 取出該房間所有訂單資料
  ...
  bookingsOfRoom.forEach(bookingOfRoom => {
    ...
    if (
      ...
      dayjs(checkin).isSame(checkoutDate) ||
      dayjs(checkout).isSame(bookingDate) ||
...
const bedCreater = async (tenantName, email, phone, checkin, checkout, adults, kids, totalPrice, currentUserId, roomId) => {
